    ...
    <main class="flex-1 flex overflow-hidden px-4 pb-4 pt-2 gap-4">
      ...
      >
        ...
        <div class="flex-1 overflow-y-auto custom-scrollbar p-2">
          ...
          <div
            v-else-if="groupedArticles.length === 0"
          ...
          <div v-else class="space-y-1">
            <div v-for="group in groupedArticles" :key="group.sourceName" class="space-y-1">
              ...
              >
                <i
                  :class="[
                    ...
                    expandedGroups.has(group.sourceName) ? 'rotate-90' : '',
                  ]"
                ></i>
                <i class="fa-solid fa-globe text-xs text-blue-400/70"></i>
                <span class="text-xs font-medium truncate flex-1">{{
                  group.sourceName || '未知来源'
                }}</span>
                <span class="text-[10px] bg-slate-800 px-1.5 rounded text-gray-500">{{
              ...
              <!-- Articles (Level 2) -->
              <div
                v-if="expandedGroups.has(group.sourceName)"
                class="ml-4 space-y-0.5 border-l border-white/5 pl-1"
              >
                <div
                ...
                >
                  <div class="flex items-center gap-1.5">
                    <span
                      ...
                      :class="[
                        'px-1 rounded-[2px] text-[9px] flex-shrink-0 min-w-[48px] text-center',
                        getCategoryMeta(article.categories[0]).bg,
                        getCategoryMeta(article.categories[0]).color,
                      ]"
                    >
                    ...
                    <span
                      ...
                      :class="[
                        'px-1 rounded-[2px] text-[9px] flex-shrink-0 min-w-[48px] text-center',
                        getCategoryMeta(undefined).bg,
                        getCategoryMeta(undefined).color,
                      ]"
                    >
                      {{ getCategoryMeta(undefined).name }}
                    </span>
                    <span
                      class="px-1 rounded-[2px] text-[9px] text-gray-400 border border-white/10 bg-white/5"
                    >
                  ...
                  <div
                    class="text-[10px] text-gray-500 flex items-center justify-between mt-0.5 px-0.5"
                  >
                    ...
                    <div class="flex items-center gap-2">
                      <div class="flex items-center gap-1">
                        ...
                        <span class="opacity-60"
                          >{{ formatTimeHour(article.created_at) }} 入库</span
                        >
                      </div>
                      <i
                        v-if="article.screenshot_path"
                        class="fa-solid fa-image text-[9px] text-brand-400/50"
                      ></i>
                    </div>
                  </div>
                </div>
            ...
            <div
              class="pt-4 pb-2 px-2 flex items-center justify-between border-t border-white/5 mt-2"
            >
              <button
      ...
      <section
        class="w-2/5 glass-card rounded-xl flex flex-col overflow-hidden border border-white/10"
      >
        ...
        <div class="flex-1 overflow-y-auto custom-scrollbar p-6 bg-slate-900/30">
          <div
            ...
            class="flex flex-col items-center justify-center h-full text-gray-600 gap-3"
          >
          ...
          <template v-else>
            <div v-if="zoneBView === 'text'" class="prose prose-invert prose-sm max-w-none">
              ...
              <div
                ...
                class="text-gray-300 leading-relaxed whitespace-pre-wrap font-serif text-base"
              >
              ...
              <div
                v-else-if="selectedArticle.full_text_cn"
                class="flex flex-col items-center justify-center py-20 bg-black/20 rounded-xl border border-white/5 gap-4"
              >
                <i class="fa-solid fa-language text-4xl text-gray-700"></i>
                <div class="text-center px-6">
                  <p class="text-gray-400 text-sm mb-2">未保存原文文本内容</p>
                  <p class="text-[11px] text-gray-500 mb-4">
                    系统已成功抓取并分析该文章（详见右侧分析结果），但未在本地保留原始英文文本。
                  </p>
                  <a
                    :href="selectedArticle.url"
                    target="_blank"
                    class="inline-flex items-center gap-2 px-4 py-2 bg-brand-500/20 text-brand-400 rounded-lg border border-brand-500/30 hover:bg-brand-500/30 transition-colors text-sm"
                  >
              ...
              >
                ...
                <div class="text-center px-6">
                  ...
                  <p class="text-[11px] text-gray-500 mb-4">
                    该文章可能由于动态加载、反爬虫限制或证书问题导致无法提取正文。
                  </p>
                  <a
                    :href="selectedArticle.url"
                    target="_blank"
            ...
            <div v-else class="h-full flex items-start justify-center">
              <div
                v-if="!selectedArticle.screenshot_path"
                class="flex flex-col items-center justify-center h-full w-full py-20 bg-black/20 rounded-xl border border-white/5 gap-4"
              >
                <i class="fa-solid fa-image-slash text-4xl text-gray-700"></i>
                <div class="text-center">
                  <p class="text-gray-400 text-sm mb-2">该文章采集时未生成截图</p>
                  <p class="text-[10px] text-gray-600 max-w-[200px] mx-auto">
                    截图通常由多模态视觉模型在分析阶段生成，部分文章可能因加载超时跳过截图。
                  </p>
                </div>
              </div>
              <a-image
      ...
      >
        ...
        <div class="flex-1 overflow-y-auto custom-scrollbar p-5 space-y-6 bg-slate-900/10">
          ...
          <template v-else>
            ...
            <div
              v-if="!selectedArticle.content"
              class="bg-amber-500/10 border border-amber-500/20 rounded-lg p-2 flex items-center gap-2 mb-4"
            >
              <i class="fa-solid fa-circle-info text-amber-400 text-xs"></i>
              <span class="text-[10px] text-amber-300/80"
                >提示：该文章正文提取失败，下方分析结果基于标题及元数据生成。</span
              >
            </div>
            ...
            <div class="space-y-3">
              ...
              <div class="bg-white/5 rounded-xl p-4 border border-white/5 space-y-4">
                <div>
                  <h5 class="text-sm font-bold text-brand-300 mb-1.5">
                    {{ selectedArticle.title_cn }}
                  </h5>
                  <p class="text-xs text-gray-400 leading-relaxed italic">
                    摘要：{{ selectedArticle.summary_cn }}
                  </p>
                </div>
            ...
            <div class="space-y-3">
              ...
              <div
                class="bg-blue-500/5 rounded-xl p-4 border border-blue-500/10 grid grid-cols-2 gap-y-4 gap-x-6"
              >
                <div class="space-y-1">
                  <span class="text-[10px] text-gray-500 block">发布时间</span>
                  <span class="text-xs text-gray-300 font-mono">{{
                    selectedArticle.pub_date || '未知'
                  }}</span>
                </div>
                <div class="space-y-1">
                ...
                <div class="space-y-1">
                  ...
                  <span
                    ...
                    :class="[
                      ...
                      getCategoryMeta(selectedArticle.categories[0]).bg,
                      getCategoryMeta(selectedArticle.categories[0]).color,
                    ]"
                  >
                ...
                <div class="space-y-1">
                  ...
                  <span class="text-xs text-gray-300 truncate block">{{
                ...
                <div class="space-y-1">
                  ...
                  <span class="text-xs text-gray-300">{{
                    getSourceTypeLabel(selectedArticle.source_type)
                  }}</span>
                </div>
              </div>
            </div>
        ...
        <div class="w-1/5 border-r border-white/5 pr-4 overflow-y-auto custom-scrollbar">
          ...
          >
            <div class="flex items-center gap-2">
              ...
              <span
                >{{ run.timestamp }}
                <span class="text-xs text-gray-500">{{ run.summary }}</span></span
              >
            </div>
          </div>
        </div>
        ...
        <div class="w-4/5 pl-4 flex flex-col gap-4 overflow-hidden">
          ...
          <div v-if="selectedRunStats.sources.length > 0" class="flex-shrink-0">
              ...
              <div class="flex items-center gap-4 text-xs text-gray-400">
                <span
                  ...
                  <span class="text-white font-mono">{{
                    selectedRunStats.total_sources
                  }}</span></span
                >
                <span
                  >总爬取:
                  <span class="text-blue-400 font-mono">{{
                    selectedRunStats.total_fetched
                  }}</span></span
                >
                <span
                  >总入库:
                  <span class="text-green-400 font-mono">{{
                    selectedRunStats.total_inserted
                  }}</span></span
                >
              </div>
            </div>
            <div class="bg-black/20 rounded-lg border border-white/5 overflow-hidden">
              <div
                class="grid grid-cols-[50px_1fr_80px_80px_1fr] gap-2 text-[11px] text-gray-500 border-b border-white/10 p-2 bg-white/5"
              >
              ...
              <div class="max-h-[200px] overflow-y-auto custom-scrollbar">
                ...
                >
                  ...
                  <span
                    ...
                    :class="stat.fetched > 0 ? 'text-blue-400' : 'text-gray-500'"
                    >{{ stat.fetched }}</span
                  >
                  <span
                    class="text-center font-mono"
                    :class="stat.inserted > 0 ? 'text-green-400' : 'text-gray-500'"
                    >{{ stat.inserted }}</span
                  >
                  <span class="text-gray-500 text-[11px] truncate" :title="stat.remark">{{
                    stat.remark || '-'
                  }}</span>
                </div>
              </div>
...
const dateRange = ref<[Dayjs, Dayjs]>([dayjs().startOf('day'), dayjs().endOf('day')])
...
function highlightKeepRows(html: string) {
  try {
    ...
    tables.forEach((table) => {
      ...
      const keepIndex = headerCells.findIndex(
        (cell) => (cell.textContent || '').trim() === '是否保留'
      )
