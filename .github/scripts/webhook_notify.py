...
CI 脚本自包含（与 AnGIneer 同款结构）：
- 仅允许 https + qyapi.weixin.qq.com 白名单，禁私网解析、禁重定向
- 卡片含版本行（backend/VERSION -> package.json -> git describe）、本次提交列表、服务状态
- 超 4096 字节自动从提交列表尾部回退截断
- 解析响应 errcode，失败则非零退出使 CI 步骤可见
- DEPLOY_STATUS 非 success 时标题切换为 ❌ 部署失败
用法：python3 webhook_notify.py [--dry-run]
"""
import ipaddress
import json
import os
...
def _send_markdown_ci(webhook: str, content: str) -> None:
    """CI 专用：发送 Markdown 消息，禁止重定向，校验 errcode。"""
    payload = json.dumps(
...
def _release_version() -> str:
    """版本号来源：backend/VERSION -> frontend/package.json -> git describe"""
    try:
        with open(os.path.join(repo, "backend", "VERSION"), encoding="utf-8") as fh:
        ...
        pass
    try:
        with open(os.path.join(repo, "frontend", "package.json"), encoding="utf-8") as fh:
...
title = "✅ DredgeScope 部署完成" if ok else "❌ DredgeScope 部署失败"
content_parts = [f"## {title}", f"> **版本:** `{_release_version()}`"]
...
META_PREFIXES = ("> **", "> 前端", "> API", "> 健康检查", "> 分支", "> 时间", "> 提交", "> 本次", "> …")
content = "\n".join(content_parts)
while len(content.encode("utf-8")) > WECOM_CONTENT_MAX_BYTES:
    commit_idx = next(
        (i for i, p in enumerate(content_parts) if p.startswith("> ") and len(p) > 8 and not p.startswith(META_PREFIXES)),
        None,
    )
